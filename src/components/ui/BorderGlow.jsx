...
function parseHSL(hslStr) {
  // Add support for quick hex conversions just in case
  if (hslStr.toUpperCase() === '#C9A96E') return { h: 39, s: 44, l: 61 };
  if (hslStr.toUpperCase() === '#E85D26') return { h: 17, s: 80, l: 53 };

  const match = hslStr.match(/([\d.]+)\s*([\d.]+)%?\s*([\d.]+)%?/);
  if (!match) return { h: 39, s: 44, l: 61 }; // Default #C9A96E
